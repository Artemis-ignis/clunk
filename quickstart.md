...
원격 에이전트는 HTTPS MCP, 로컬 파일은 stdio를 사용합니다. [에이전트 연결 화면](https://clunk.games/agents)에서 키를 발급하면 클라이언트별 설정이 완성됩니다.
...
원격으로 붙일 때는 `/agents`에서 발급한 endpoint와 Bearer 키를 넣습니다.
