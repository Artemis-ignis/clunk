...
## Engine-fit agent flow (v0.2)

When the user asks whether an asset works "for my game" (or hands you generated assets):

1. Engine unknown → call `clunk_engine_profiles`, show the presets (Godot/Unity/Unreal x
   mobile/desktop, budgets + import caveats) and **ask the user to choose** — or ask for
   reference assets that already work in their game and call `clunk_profile_from` to derive
   their project profile.
2. Judge with `enginePreset:<key>` (or the derived `profileFile`) so the verdict reads
   against THEIR game.
3. Not READY → `clunk_optimize` first (safe allowlisted cleanups). Budget findings beyond
   that (triangles/materials/textures) mean the asset itself must change: when you authored
   it (img2threejs-style procedural code), edit the factory and re-export, then re-inspect.
   Loop until READY. Never overwrite sources.
4. Report score + findings + which profile judged it, and offer the Passport as proof.
