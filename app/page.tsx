  ...
  return (
    <div className="site-shell clunk-v5">
      ...
      <main className="landing-v5">
        ...
        <section className="landing-v5-final" id="start"><div><span className="eyebrow">CREATE · REVIEW · SELL</span><h2>실제 제작부터<br /><em>마켓 등록까지.</em></h2><p>공개 샘플은 바로 보고, 직접 만든 2D/3D 파일은 로그인한 Studio에서 생성·검사·검수·판매 Draft로 이어갑니다.</p></div><div className="landing-v5-actions"><Link className="button button-primary" href="/studio" prefetch={false}>실제 제작 시작 <Icon name="arrowRight" size={15} /></Link><Link className="button button-quiet" href="/marketplace" prefetch={false}>에셋 마켓 보기 <Icon name="arrowUpRight" size={15} /></Link></div></section>
        <footer className="landing-v5-footer"><div className="site-footer-brand"><span className="brand-mark"><BrandMark size={30} gradientId="clunk-v5-footer" /></span><div><strong>Clunk</strong><span>2D + 3D 에셋 품질·근거 게이트</span></div></div><nav className="site-footer-nav" aria-label="사이트 링크"><Link href="/app" prefetch={false}>검사기 · 로그인</Link><Link href="/studio" prefetch={false}>실제 제작</Link><Link href="/marketplace" prefetch={false}>에셋 마켓</Link><Link href="/connect" prefetch={false}>에이전트 연결</Link><Link href="/dashboard" prefetch={false}>대시보드</Link><Link href="/docs" prefetch={false}>문서</Link><a href="/llms.txt">llms.txt</a></nav><span className="demo-marker">DEMO MODE · 실제 결제 아님</span></footer>
      </main>
    </div>
  );
}
