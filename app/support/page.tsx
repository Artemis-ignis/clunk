...
const LIMITS = [
  ...
  {
    title: "서버 검증은 직접 요청할 때만 일어납니다",
    body: "기본 흐름에서 검사는 이용자의 기기에서만 실행되고, 그 결과로 남는 로컬 기록은 같은 파일로 누구나 재현해 대조할 수 있는 기록이지 제3자에게 내미는 증명서가 아닙니다. 제출용 서명 문서가 필요하면 ‘서버 검증’을 직접 선택하시면 되고, 그때만 해당 파일의 바이트가 서버로 올라가 검사되며 검사 직후 폐기합니다. 이 서명은 Clunk가 그 바이트를 직접 검사했다는 확인이며, 독립 감사기관의 인증은 아닙니다.",
  },
  {
...
const FAQ = [
  {
    q: "내 에셋이 서버로 올라가나요?",
    a: "기본적으로는 올라가지 않습니다. 검사와 최적화는 브라우저 안에서 실행되고, 서버에는 파일 이름·형식·바이트 길이·해시와 검사 결과만 저장됩니다. 예외는 하나입니다. ‘서버 검증 Passport’를 직접 요청하시면 그 파일의 바이트가 업로드되어 서버가 직접 검사합니다. 그때도 바이트는 검사 직후 폐기하며, 원본 바이트를 저장하는 테이블 자체가 없습니다.",
  },
  {
    q: "내 에셋이 AI 학습에 쓰이나요?",
    a: "쓰지 않습니다. 기본 흐름에서는 원본이 서버로 전송되지도 않고, 서버 검증으로 업로드된 바이트도 검사 직후 폐기합니다. 검사 결과 역시 서비스 제공과 장애 대응 외의 목적으로 사용하지 않습니다.",
  },
  {
    q: "원본 파일이 바뀌나요?",
