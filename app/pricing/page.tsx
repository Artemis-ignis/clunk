...
export const metadata: Metadata = {
  title: "요금 안내",
  description: "월정액 플랜과 선불 크레딧 팩을 함께 쓰는 Clunk 요금 구조입니다. 지금은 요금이 청구되지 않습니다.",
};
    ...
    <SiteShell active="pricing">
      <main className="page">
          ...
          <div className="plan-grid">
            {MONTHLY_PLANS.map((plan) => (
              <article key={plan.name} className={`plan-card${plan.active ? " plan-card-active" : ""}`}>
                ...
                {!plan.active ? <span className="plan-tier-note">국내 결제 연동과 함께 오픈</span> : null}
              </article>
            ))}
          </div>
        </section>
        ...
        <section className="pricing-block" aria-label="크레딧 규칙">
          <div className="pricing-block-head">
            <h2>크레딧이 세는 것</h2>
            <p>아래 규칙은 지금 워크스페이스에서 실제로 그대로 동작합니다.</p>
          </div>
          <div className="pack-grid">
            ...
            <article className="pack-card">
              ...
              <p className="pack-note">샘플 실행은 크레딧과 워크스페이스 이력에서 제외됩니다.</p>
            </article>
          </div>
        </section>
        ...
        <aside className="callout">
          ...
          <p>
            국내 결제 연동 전이라 지금은 어떤 카드도 청구되지 않습니다. 워크스페이스의 크레딧은 실제로
            동작하는 원장이며, 유료 전환 시에는 요금표와 개정 약관을 먼저 공지하고 동의를 받은 뒤에만
            과금합니다.
          </p>
        </aside>
