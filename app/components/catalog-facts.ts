 ...
 */
/**
 * 등급마다 한 줄. 한 문장으로 이어 붙이면 가운뎃점 다섯 개가 붙은 한 덩어리가 되어
 * 아무도 안 읽는다 — 2026-09-04 마스터가 요금 화면에서 그 줄을 짚었다. 규칙이 세 줄이면
 * 세 줄로 보여 준다.
 */
export const GRADE_ROWS = [
  { letter: "S", when: "움직이는 동작이 있고 폴리곤 1,500개 이상이거나, 폴리곤 4,000개 이상" },
  { letter: "A", when: "움직이는 동작이 있거나, 폴리곤 1,500개 이상이거나, 여러 모델을 묶은 것" },
  { letter: "B", when: "그 밖의 모델과 텍스처, 스프라이트 시트" },
] as const;

/** 등급과 받을 수 있는지는 다른 이야기다. 한 줄 띄어 따로 적는다. */
export const GRADE_ACCESS = "B 등급은 로그인만 하면 받습니다. A와 S는 구독자가 받습니다.";

/** 한 줄로 적어야 하는 자리(에이전트 응답, 검사 메시지)를 위한 같은 내용. */
export const GRADE_RULE = `등급 기준: ${GRADE_ROWS.map((row) => `${row.letter}는 ${row.when}`).join(". ")}. ${GRADE_ACCESS}`;
