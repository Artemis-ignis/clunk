...
export function CreditPacksPanel() {
  ...
  if (state === "loading") {
    return <p className="credit-packs-state" role="status">크레딧 팩 상태를 불러오는 중입니다…</p>;
  }
  if (state === "error" || !packs) {
    return <p className="credit-packs-state" role="alert">크레딧 팩 상태를 불러오지 못했습니다. API 응답이 없으면 아무 가격도 표시하지 않습니다.</p>;
  }

  const anyPurchasable = packs.some((pack) => pack.purchasable);

  return (
    <div className="credit-packs" data-testid="credit-packs-panel">
      <div className="credit-packs-grid">
        {packs.map((pack) => (
          <article className="credit-pack-card" key={pack.id} data-status={pack.status}>
            <span className="credit-pack-name">{pack.name}</span>
            <strong className="credit-pack-credits">{pack.credits.toLocaleString("ko-KR")} 크레딧</strong>
            {pack.purchasable ? (
              <>
                <span className="credit-pack-price">{formatPackPrice(pack)}</span>
                <button type="button" className="button button-primary button-sm" onClick={() => void buy(pack)} disabled={!consent}>
                  {consent ? "구매하기" : "동의 후 구매 가능"}
                </button>
              </>
            ) : (
              <span className="credit-pack-pending">가격 확정 전 — 아직 구매할 수 없습니다</span>
            )}
          </article>
        ))}
      </div>
      {anyPurchasable ? (
        <label className="credit-packs-consent">
        ...
        <p className="credit-packs-state">
          팩 구성은 확정되어 있고, 판매 가격은 운영자가 확정하는 즉시 이 자리에서 실값으로 열립니다.
        </p>
      )}
      {message ? <p className="credit-packs-state" role="status">{message}</p> : null}
    </div>
  );
}
