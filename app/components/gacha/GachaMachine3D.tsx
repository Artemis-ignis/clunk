...
export function GachaMachine3D() {
  ...
  /* 로그인 여부 -------------------------------------------------------------
     잔액은 여기서 읽지 않는다 — 잔액을 들고 다니던 내비의 동전 지갑은 없어졌다
     (2026-09-04). 이 화면이 로그인 여부를 아는 것은 "받기" 단추 때문이다. */
  useEffect(() => {
    let alive = true;
  ...
  /**
   * 잔액은 어느 화면도 들지 않는다. 내비에 걸려 있던 동전 지갑은 2026-09-04 에 없어졌다 —
   * 현금으로 코인을 사서 상품과 바꾸는 그림이 결제대행 심사가 막은 바로 그것이었다.
   * 무대에는 기계와 글줄만 남는다.
   */
