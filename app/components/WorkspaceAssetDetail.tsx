    ...
    <div className="workspace-asset-detail" data-testid="workspace-asset-detail">
      <div className="workspace-asset-breadcrumb"><Link href="/dashboard">작업공간</Link><Icon name="chevronRight" size={13} /><span>{asset.fileName}</span></div>
       <header className="workspace-asset-header">
        ...
        <div className="workspace-asset-header-actions"><Link className="button button-primary button-sm" href={`/studio?source_asset_id=${encodeURIComponent(asset.id)}`}>이 결과로 새 버전 만들기 <Icon name="arrowUpRight" size={13} /></Link><Link className="button button-quiet button-sm" href="/kits">모음집에 담기 <Icon name="boxes" size={13} /></Link></div>
      </header>
      ...
      {payload.kits?.length ? <section className="workspace-asset-kits"><span className="mono-label">묶음</span><h3>이 파일이 들어간 모음집</h3><div>{payload.kits.map((kit) => <Link className="workspace-kit-link" href={`/kits?kit=${encodeURIComponent(kit.id)}`} key={kit.id}><strong>{kit.title}</strong><span>{kit.status}</span><Icon name="arrowRight" size={13} /></Link>)}</div></section> : null}
    </div>
  );
}
