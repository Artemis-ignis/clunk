...
export function WorkspaceAssetDetail({ assetId }: { assetId: string }) {
  ...
  if (state === "error" || !payload?.asset) return <div className="workspace-asset-state workspace-asset-state-error"><Icon name="triangleAlert" size={22} /><strong>에셋 기록을 열 수 없습니다.</strong><small>{message}</small><Link className="button button-quiet button-sm" href="/dashboard">Workspace로 돌아가기 <Icon name="arrowLeft" size={13} /></Link></div>;
  ...
  return (
    <div className="workspace-asset-detail" data-testid="workspace-asset-detail">
      <div className="workspace-asset-breadcrumb"><Link href="/dashboard">Workspace</Link><Icon name="chevronRight" size={13} /><span>{asset.fileName}</span></div>
       <header className="workspace-asset-header">
         <div><span className="mono-label">WORKSPACE ASSET · {asset.format.toUpperCase()}</span><h2>{asset.fileName}</h2><p>현재 Workspace API가 반환한 metadata·artifact hash·검수 상태를 확인합니다. 저장되지 않은 결과는 다운로드 성공으로 표시하지 않습니다.</p></div>
...
export function WorkspaceAssetLibrary() {
  ...
  const [state, setState] = useState<"loading" | "ready" | "error">("loading");
  const [message, setMessage] = useState("실제 Workspace 에셋을 불러오는 중입니다.");
  ...
  return (
    <div className="workspace-asset-library" data-testid="workspace-asset-library">
      <section className="ws-welcome">
        <div><span className="mono-label">PRIVATE ASSET INDEX · /api/generation + /api/runs</span><h2>내 에셋을<br /><em>실제 기록으로 찾습니다.</em></h2><p>생성 작업과 검사 API가 반환한 assetId만 모읍니다. 저장된 artifact·검수·Passport 상태는 각 상세 화면에서 다시 확인합니다.</p></div>
        <Link className="button button-primary" href="/studio">내 Workspace에서 생성 <Icon name="arrowUpRight" size={15} /></Link>
      </section>
      ...
      <section className="panel" aria-labelledby="asset-library-heading">
        <div className="panel-head"><div><span className="mono-label">ASSET LIBRARY</span><h3 id="asset-library-heading">저장된 에셋 {state === "loading" ? "" : `${items.length}건`}</h3></div><span className="mono-label">실제 응답만</span></div>
        <div className="ws-toolbar-row"><label className="creation-field"><span>검색</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="파일명·asset id·종류" /></label><label className="creation-field"><span>종류</span><select value={kind} onChange={(event) => setKind(event.target.value)}>{kinds.map((value) => <option value={value} key={value}>{value === "all" ? "전체 종류" : value}</option>)}</select></label><span className="ws-spacer" /><span className="ws-hint">{state === "ready" ? `${visibleItems.length}건 표시` : ""}</span></div>
        {state === "loading" ? <div className="empty-block"><span className="spinner" /><strong>에셋 인덱스를 준비하는 중입니다</strong></div> : visibleItems.length ? <div className="project-list">{visibleItems.map((item) => <Link className="project-row" href={`/assets/${encodeURIComponent(item.id)}`} key={item.id}><Icon name="box" size={17} /><span><strong>{item.fileName}</strong><small>{item.assetKind} · {item.format.toUpperCase()} · {item.storageStatus} · {item.createdAt}</small></span><span className="mono-label">{item.status}</span><Icon name="arrowRight" size={13} /></Link>)}</div> : <div className="empty-block empty-block-lg"><Icon name="box" size={24} /><strong>{items.length ? "검색 결과가 없습니다" : "아직 저장된 에셋이 없습니다"}</strong><p>{items.length ? "검색어 또는 종류 필터를 바꿔 보세요." : "Studio에서 실제 생성 작업을 실행하거나 검사기에서 에셋을 검사하면 이곳에 표시됩니다."}</p>{!items.length ? <Link className="button button-quiet button-sm" href="/studio">첫 생성 실행 <Icon name="arrowRight" size={13} /></Link> : null}</div>}
      </section>
    </div>
  );
}
