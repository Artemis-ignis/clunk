  ...
  return (
    ...
    <div className="cv5 cv5-surface agents-cv5">
      ...
      <main className="agents-page agents-v4-page">
        ...
        <section className="agents-v4-section agents-v4-boundary" aria-labelledby="boundary-heading"><div className="agents-v4-heading"><span className="eyebrow">결과를 바르게 읽는 법</span><h2 id="boundary-heading">파일 검사 통과와<br /><em>화면 통과는 다릅니다.</em></h2><p>Clunk의 검사 규칙({RULE_SET.id})이 확인하는 것은 파일 내용과 구조, 규칙 위반입니다. 엔진 렌더와 게임 시점은 Clunk가 오프라인으로 직접 그려 측정한 뒤 따로 판정합니다. 파일 검사 점수가 그 판정을 대신하지 않습니다.</p><Link className="text-link" href="/docs/contracts">규칙과 상태 보기 <Icon name="arrowRight" size={14} /></Link></div><div className="agents-v4-statuses"><article><span>파일 검사</span><strong>형식 · 예산 · 물리</strong><small>형식과 예산, 재질·텍스처, 부품이 놓인 자리(접지·부양·관통)를 규칙으로 측정</small></article><article><span>엔진 렌더</span><strong>4각도 렌더</strong><small>3/4·정면·측면·위를 오프라인으로 그려 접지·노출·팔레트를 측정</small></article><article><span>게임 시점</span><strong>눈높이 5 m · 15 m</strong><small>플레이어 카메라 높이에서 실루엣과 46 px 가독성을 측정</small></article><article><span>판정</span><strong>기계가 냅니다</strong><small>네 칸을 모아 자동으로 판정하고, 사람 검토는 필요할 때만 권합니다</small></article></div><div className="agents-v4-profile-strip">{TARGET_PROFILES.slice(0, 5).map((profile) => <span key={profile.id}>{profileLabel(profile)}</span>)}</div></section>

        <section className="agents-v4-handoff" aria-labelledby="handoff-heading"><div><span className="eyebrow">판정까지 가는 길</span><h2 id="handoff-heading">파일 검사는<br /><em>네 단계 중 첫 칸입니다.</em></h2><p>파일 검사는 규격을 봅니다. 게임에 넣어도 되는지는 엔진 렌더와 게임 시점까지 Clunk가 직접 그려 측정한 뒤 판정합니다. 측정하지 못한 칸은 비어 있다고 적고, 채워진 것처럼 바꾸지 않습니다.</p><code className="agents-v4-unavailable">돌려 보지 않은 엔진은 &quot;확인할 환경 없음&quot;으로 남고, 통과로 바뀌지 않습니다.</code><div className="agents-v4-handoff-actions"><Link className="button button-primary button-sm" href="/studio">에셋 제작 열기 <Icon name="arrowUpRight" size={14} /></Link><Link className="button button-quiet button-sm" href="/marketplace">에셋 마켓 보기 <Icon name="arrowRight" size={14} /></Link></div></div><div className="agents-v4-handoff-card">{HANDOFF_LANES.map(([label, value]) => <div key={label}><span>{label}</span><b>{value}</b></div>)}</div></section>
