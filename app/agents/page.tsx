  ...
  return (
    <SiteShell active="agents">
      <main className="agents-page agents-v4-page">
        ...
        <section className="agents-v4-section agents-v4-boundary" aria-labelledby="boundary-heading"><div className="agents-v4-heading"><span className="eyebrow">READ THE RESULT CORRECTLY</span><h2 id="boundary-heading">구조 PASS와<br /><em>화면 PASS는 다릅니다.</em></h2><p>{RULE_SET.id}는 hash·parser·policy를 증명합니다. 실제 shipped frame과 사람의 판단은 자동으로 승격하지 않습니다.</p><Link className="text-link" href="/docs#contracts">계약과 상태 보기 <Icon name="arrowRight" size={14} /></Link></div><div className="agents-v4-statuses"><article><span>STATIC / TECHNICAL</span><strong>PASS</strong><small>bytes · hash · policy · blocker</small></article><article><span>VISUAL RUNTIME</span><strong>GAP</strong><small>shipped renderer frame 필요</small></article><article><span>HUMAN REVIEW</span><strong>PENDING</strong><small>사람의 화면 판정 대기</small></article></div><div className="agents-v4-profile-strip">{TARGET_PROFILES.slice(0, 5).map((profile) => <span key={profile.id}>{profile.label}</span>)}</div></section>
        ...
        <section className="agents-v4-final"><div><span className="eyebrow">NEXT</span><h2>실제 에셋을<br /><em>한 번 호출해 보세요.</em></h2></div><div className="agents-v4-actions"><a className="button button-primary" href="#connect">클라이언트 설정하기 <Icon name="arrowUpRight" size={15} /></a><Link className="button button-quiet" href="/app">샘플 GLB 검사 <Icon name="arrowRight" size={15} /></Link></div></section>
      </main>
    </SiteShell>
  );
}
