  ...
  return (
    ...
    <div className="cv5 cv5-surface agents-cv5">
      ...
      <main className="agents-page agents-v4-page">
        <section className="agents-v4-hero public-hero-frame public-hero-agents"><div className="agents-v4-copy"><div className="hero-status-line"><span className="status-dot status-dot-on" /><span>Clunk 연결 서버</span><code>v{MCP_SERVER.version}</code></div><span className="eyebrow">AI 도구 연결(MCP)</span><h1>생성 직후,<br /><em>에이전트가 검사합니다.</em></h1><p>Clunk가 직접 운영하는 연결 서버를 한 번 걸어 두면 Claude Code, Codex, Cursor, GitHub Copilot, Claude Desktop, VS Code가 이 웹사이트와 똑같은 규칙으로 에셋을 고르고 검사합니다. 브라우저를 열지 않고도 마켓을 뒤지고, 파일을 올려 검사하고, 같은 검사 기록을 받아 갑니다.</p><div className="agents-v4-actions"><a className="button button-primary" href="#connect">연결 시작 <Icon name="chevronDown" size={15} /></a><Link className="button button-quiet" href="/docs/quickstart">설정 가이드 <Icon name="arrowRight" size={15} /></Link></div><div className="agents-v4-proof"><span>웹으로 바로 쓰는 도구 <b>{MCP_HTTP_TOOL_COUNT}</b>개</span><span>내 컴퓨터에서 쓰는 도구 <b>{MCP_TOOLS.length}</b>개</span><span>원본 파일 덮어쓰기 <b>0</b></span></div></div><LiveEvidenceShowcase variant="agents" compact /></section>
