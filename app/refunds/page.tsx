...
/**
 * 취소·환불정책. 크레딧 차감 규칙과 환불 반영 절차는 실제 동작(성공했을 때만 차감,
 * 환불 알림을 받으면 주문을 환불 완료로, 그 에셋을 받을 권리를 회수됨으로 바꾼다)에서
 * 옮겼습니다. 화면에는 그 상태를 한국어로 적습니다 — 영문 상태값은 데이터베이스의
 * 사정이지 구매자가 읽어야 할 말이 아닙니다.
 */
export default function RefundsPage() {
  return (
    <LegalShell
      eyebrow="취소와 환불"
      title="취소·환불정책"
      lede="Clunk 마켓에서 구매한 디지털 에셋과 작업공간에서 쓰는 실행 크레딧에 대한 청약철회·취소·환불 기준입니다. 디지털 콘텐츠 특성상 제공이 개시된 뒤에는 청약철회가 제한될 수 있어, 그 조건을 결제 전에 고지합니다."
    >
      ...
      <section aria-labelledby="refunds-scope">
        ...
        <ul>
          <li><b>마켓 단건 구매</b> — 공개된 마켓 상품의 에셋 파일을 한 번 결제해 사고, 내려받을 권리를 받는 거래</li>
          <li><b>실행 크레딧</b> — 검사·수정·에셋 만들기를 한 번 실행할 때 쓰이는 내부 사용 단위</li>
        </ul>
        <p>
      ...
      <section aria-labelledby="refunds-eligible">
        ...
        <ul>
          <li>결제는 끝났는데 <b>받을 권리가 주어지지 않아 내려받을 수 없는 경우</b></li>
      ...
      <section aria-labelledby="refunds-credit">
        ...
        <ul>
          ...
          <li>같은 요청을 두 번 보내도 한 번만 기록되므로 두 번 차감되지 않습니다.</li>
          <li>실행이 성공해 결과물이 작업공간에 저장된 경우, 그 크레딧은 제공이 끝난 것으로 봅니다.</li>
