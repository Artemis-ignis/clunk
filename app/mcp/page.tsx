      ...
      <main className="mcp-page">
        <section className="mcp-page-hero">
          <div className="mcp-page-copy">
            ...
            <p>Clunk endpoint를 연결하면 Claude Code, Codex, Cursor와 같은 클라이언트가 같은 검사 계약과 상태 경계를 사용합니다. HTTP는 로컬 경로를 읽지 않고, 업로드한 바이트와 검증된 evidence만 받습니다.</p>
        ...
        <section className="mcp-page-steps" aria-label="MCP 연결 순서">
          <article><span>01</span><strong>키를 발급합니다</strong><p>workspace 범위의 Bearer key를 한 번 표시합니다.</p></article>
          <article><span>02</span><strong>클라이언트를 등록합니다</strong><p>endpoint와 인증 헤더가 채워진 설정을 복사합니다.</p></article>
          <article><span>03</span><strong>initialize → tools/list</strong><p>실제 handshake가 응답한 도구만 연결 완료로 표시합니다.</p></article>
        </section>

        <section className="mcp-page-boundary">
          <div>
            <span className="eyebrow">BOUNDARY</span>
            <h2>연결 PASS가<br /><em>게임 투입 승인은 아닙니다.</em></h2>
            <p>HTTP MCP는 연결과 구조 evidence를 전달합니다. player-facing 캡처와 human decision은 별도 lane으로 남기며 자동으로 승격하지 않습니다.</p>
          </div>
