...
export const MCP_HTTP_TOOLS = [
  ...
  },
  {
    ...
    description:
      "Inspect uploaded asset bytes against one engine's target profile and return the measured evidence: triangle and draw-call counts, materials, texture memory, bounding box, and every rule finding. For a 3D model the findings also cover physical plausibility, measured in world space with every parent transform applied: how far the scene's lowest point sits from the y=0 ground plane (GEO-GROUND-CONTACT), named parts that touch neither the ground nor any other part and the millimetre gap to the nearest one (GEO-FLOATING-PART), parts whose triangles actually intersect with the penetration depth in millimetres and, for an animated file, the clip and phase where it is deepest (GEO-PART-INTERSECTION, 8 phases sampled per clip), two differently named parts that actually cross each other, with the size of the overlapping region in millimetres (GEO-PART-PENETRATION), zero-thickness single-sided cards (GEO-THIN-SHELL), animation channels that drive node scale (SCENE-ANIMATED-SCALE), unnamed mesh nodes (SCENE-UNNAMED-MESH) and declared extensionsRequired (FORMAT-EXTENSION-REQUIRED). Only one of those is a hard blocker: GEO-PART-PENETRATION reports ERROR when a wheel, tyre, roller or sprocket occupies the same volume as another part, because a wheel has to turn and turning takes it through that part in every phase. Everything else is a measurement, not a verdict: a tree's roots below ground, a shaft through its bearing and a leaf card are all intended, so they are reported for a human or agent to judge. HTTP cannot read a path on your machine — send the file itself as base64 (fileName + bytesBase64), or a multi-file bundle (entryFileName + files[]). Use Clunk's local stdio MCP when the file must stay on disk.",
