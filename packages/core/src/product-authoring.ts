...
export type ProceduralAuthoringRequest = {
  ...
  license?: string;
  /**
   * Bytes for the entry artifact, supplied by a real generator instead of drawn by the
   * recipe. The evidence below is produced from whatever ends up here, so the file that
   * is inspected is always the file that ships — passing generated bytes in is the only
...
export function createProceduralAuthoring(request: ProceduralAuthoringRequest): ProceduralAuthoringResult {
  ...
  if (request.entry) {
    ...
    artifacts[index] = makeArtifact(request.entry.fileName, "entry", request.entry.contentType, request.entry.bytes);
  }
