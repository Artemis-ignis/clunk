...
export type SpriteStaticStatus = "PASS" | "FAIL";
...
export function evaluateSpriteSheetReview(manifest: SpriteSheetReviewManifest): SpriteSheetReviewReport {
  ...
  const needsMetrics = policy.mode !== "OFF" && (
    policy.minDistinctFrameRatio !== undefined
    || policy.maxDuplicateFrameRatio !== undefined
    || policy.minMeanFrameDelta !== undefined
    ...
    || policy.minSilhouetteCoverage !== undefined
  ...
  if (metrics) {
    ...
    if (policy.minDistinctFrameRatio !== undefined && metrics.distinctFrameRatio !== undefined && metrics.distinctFrameRatio < policy.minDistinctFrameRatio) {
      qualityViolations += 1;
      issues.push(issue(
        "SPRITE-LOW-DISTINCT-FRAMES",
        enforcement === "BLOCKING" ? "BLOCKING" : "ADVISORY",
        metrics.distinctFrameRatio,
        policy.minDistinctFrameRatio,
    ...
    if (policy.maxDuplicateFrameRatio !== undefined && duplicateRatio > policy.maxDuplicateFrameRatio) {
      qualityViolations += 1;
      issues.push(issue(
        "SPRITE-DUPLICATE-FRAMES",
        enforcement === "BLOCKING" ? "BLOCKING" : "ADVISORY",
        duplicateRatio,
        policy.maxDuplicateFrameRatio,
    ...
    if (policy.minMeanFrameDelta !== undefined && metrics.meanFrameDelta !== undefined && metrics.meanFrameDelta < policy.minMeanFrameDelta) {
      qualityViolations += 1;
      issues.push(issue(
        "SPRITE-LOW-MEAN-FRAME-DELTA",
        enforcement === "BLOCKING" ? "BLOCKING" : "ADVISORY",
        metrics.meanFrameDelta,
        policy.minMeanFrameDelta,
    ...
    if (policy.minSilhouetteCoverage !== undefined) {
      if (metrics.silhouetteCoverages === undefined) {
        ...
        if (minimum < policy.minSilhouetteCoverage) {
          qualityViolations += 1;
          issues.push(issue(
            "SPRITE-SILHOUETTE-COVERAGE",
            enforcement === "BLOCKING" ? "BLOCKING" : "ADVISORY",
            minimum,
            policy.minSilhouetteCoverage,
  ...
  const staticStatus: SpriteStaticStatus = "PASS";
  ...
  const reviewStatus: SpriteReviewStatus = humanDecision !== "NOT_EVALUATED"
    ...
    : captures.length > 0
      ? "PENDING"
      : "NOT_EVALUATED";
  const readiness: SpriteReadiness = quality === "BLOCKED"
    ? "blocked"
    : quality === "UNAVAILABLE" || (policy.requireRuntimeCapture === true && captures.length === 0)
      ? "unavailable"
      : visualRuntime !== "PASS" || playerFacing !== "PASS" || quality !== "PASS"
        ? "conditional"
        : "ready";
