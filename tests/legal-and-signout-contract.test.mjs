...
test("법적 문서 3종이 200으로 렌더되고 시행 중임을 배지로 밝힌다", async () => {
  for (const pathname of LEGAL_ROUTES) {
    ...
    // 2026-09-06(마스터 결정): 표현은 "베타 기간에는 무료" 하나로 맞춘다. 옛 배지 낱말만 막는다.
    assert.ok(!html.includes("베타 무료") && !html.includes("무료 (베타)"), `${pathname}에 옛 베타 배지 표현이 남아 있습니다`);
    ...
    // 2026-09-05 점검 M12: "초안 작성일 2026-08-31" 은 이용자가 아니라 우리가 보려고 적어
    // 둔 날짜였다. 문서가 언제부터 효력이 있고 언제 마지막으로 고쳐졌는지는 위 두 줄이
    // 이미 말한다. 그 줄이 되돌아오면 실패한다.
    assert.ok(!html.includes("초안 작성일"), `${pathname}에 초안 작성일이 남아 있습니다`);
    for (const stale of [
      "초안 · 시행 전",
      "시행일 미정",
      "이 문서도 시행 전 초안입니다",
...
test("사업자 표시사항은 등록증 실값과 정확히 일치하고, 미확정 항목만 플레이스홀더다", async () => {
  for (const pathname of ["/terms", "/refunds"]) {
    ...
    assert.ok(
      html.includes("[확정 전 — 유료 판매를 시작할 때 신고하고 기재합니다]"),
...
test("결제 미개시 안내는 결제 provider 미설정 상태에서만 렌더된다", async () => {
  ...
  assert.ok(html.includes("베타 기간에는 결제를 받지 않습니다."), "결제 미설정 상태에서 안내 문장이 없습니다");
  // 2026-09-06(마스터 결정): "베타 기간에는 무료"를 모든 화면에 같은 말로 적는다.
  assert.ok(!html.includes("베타 무료") && !html.includes("무료 (베타)"), "요금 화면에 옛 베타 배지 표현이 남아 있습니다");
...
test("/login과 /signup은 서로 다른 문이고, 영문 라벨이 남아 있지 않다", async () => {
  ...
  assert.ok(
    signup.includes("Google이나 GitHub 계정으로 바로 시작합니다"),
    "가입 화면의 한 문장이 다릅니다",
  );
