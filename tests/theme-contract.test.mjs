...
test("어제의 색 리터럴이 다시 늘지 않는다", async () => {
  /* 왜 이 문턱인가.
     ...
     · legacy 층(globals·foundry·workspace·live-evidence). 이쪽은 램프를
       theme.css 가 통째로 갈아 끼우므로 리터럴이 남아도 화면은 따라온다.
       그래도 늘어나면 안 되므로 1250 으로 묶는다(현재 1189).
  ...
  assert.ok(legacy <= 1250, `legacy 층의 색 리터럴이 ${legacy}개다(상한 1250)`);
});
